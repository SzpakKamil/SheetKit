...
/// `GregorianCalendarDatePickerDelegate` is a class conforming to `SKTPickerViewDelegate`
/// that can be used with `SKTDatePickerView` to display a date picker allowing users to select a date
/// that is valid in Gregorian calendar using 3 picker components for day, month and year.
class GregorianCalendarDatePickerDelegate {
    enum DateComponentsOrder {
        case dayMonthYear
        case monthDayYear
        case yearMonthDay
    ...
    /// Returns the date that is currently set in the picker view.
    /// Also triggers calling the `onDateChanged` closure on setting new value.
    var date: Date {
        didSet {
            onDateChanged?(date)
        }
...
extension GregorianCalendarDatePickerDelegate: SKTPickerViewDelegate {
